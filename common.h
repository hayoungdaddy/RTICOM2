...
/*
#define NUM_REGEND 12
static QColor pgaColor[NUM_REGEND] = { QColor(143, 0, 0), QColor(207, 0, 0), QColor(255, 15, 0),
                                QColor(255, 81, 0), QColor(255, 143, 0),
                                QColor(255, 207, 0),  QColor(255, 255, 15), QColor(191, 255, 81),
                                QColor(95, 255, 175), QColor(0, 191, 255),
                                QColor(0, 0, 255), QColor(0, 0, 191) };
static double pgaValue[NUM_REGEND] = { 50, 10, 6,
                              2, 0.8,
                              0.4, 0.1, 0.06,
                              0.02, 0.008,
                              0.004, 0.001 };
static int pgaWidth[NUM_REGEND] = { 20, 20, 20,
                             20, 18,
                             18, 15, 15,
                             13, 12,
                             12, 12 };
...
static QColor pgaColor[NUM_REGEND] = { QColor(143, 0, 0), QColor(207, 0, 0), QColor(255, 15, 0),
                                ...
                                QColor(255, 207, 0),  QColor(255, 255, 15), QColor(191, 255, 81),
                                QColor(0, 0, 255), QColor(0, 0, 191) };
static double pgaValue[NUM_REGEND] = { 813, 813, 431,
                              235, 128,
                              66, 24, 8,
                              3, 1};
static int pgaWidth[NUM_REGEND] = { 20, 20, 20,
...
static short swapi(short x){
    return (short)((x << 8) | ((x >> 8) & 0xff));
}

static int swap(int x){
    return (int)((swapi((short)x) << 16) | (swapi((short)(x >> 16)) & 0xffff));
}
...
static QMultiMap<int, _QSCD_FOR_MULTIMAP> convertMMap(char *msg, bool isKISS)
{
    ...
    */
    ...
    for(int i=0;i<MAX_QSCD_CNT;i++)
    {
        ...
        /*
        crc suncrc = crcFast((unsigned char *)&(QSCDBlock[i].QFlag), 120-4);

        qDebug() << suncrc << (int)suncrc << (unsigned long)(swap(QSCDBlock[i].crc32)) << QSCDBlock[i].crc32 << swap(QSCDBlock[i].crc32);
        if( (int)suncrc != swap(QSCDBlock[i].crc32) )
        {
            cnt_err_crc++;
            qDebug() << (int)suncrc << swap(QSCDBlock[i].crc32);
            continue;
        }
        */
